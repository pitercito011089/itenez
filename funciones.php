...
function obtenerNombreUsuario($idUsuario) {
    ...
    $resultado = select($sentencia, $parametros);
 ...
 * @return string|null El nombre del cliente o null si no se encuentra.
 */
function obtenerNombreCliente($idCliente) {
    try {
        ...
        $consulta = $conexion->prepare("SELECT nombre FROM clientes WHERE id = :idCliente");
        $consulta->bindParam(':idCliente', $idCliente, \PDO::PARAM_INT);
        
        // Intentar ejecutar la consulta
        $consulta->execute();
        $nombreCliente = $consulta->fetchColumn();

        // Devolver el nombre del cliente o null si no se encuentra
        return ($nombreCliente !== false) ? $nombreCliente : null;
    } catch (\PDOException $e) {
        die("Error al obtener el nombre del cliente: " . $e->getMessage());
    } finally {
        // Cerrar la conexión
        $conexion = null;
    }
...
/**
 * Obtiene los detalles de una venta a partir de su ID.
 *
 * @param int $idVenta El ID de la venta.
 * @return object|false Un objeto con los detalles de la venta o false si no se encuentra.
 */
function obtenerDetallesVenta($idVenta) {
    $sentencia = "SELECT * FROM ventas WHERE id = ?";
    $venta = select($sentencia, [$idVenta]);
    
    return (!empty($venta)) ? $venta[0] : false;
}
function actualizarDetallesVenta($idVenta, $idProducto, $precioProducto) {
    try {
        $conexion = conectarBaseDatos();

        // Verificar si la venta existe
        $ventaExistente = obtenerVentaPorId($idVenta);
        ...
        $productoExistente = obtenerProductoPorId($idProducto);
        ...
        $sentencia = "UPDATE ventas SET idProducto = ?, precioProducto = ? WHERE id = ?";
        $parametros = [$idProducto, $precioProducto, $idVenta];

        $actualizado = editar($sentencia, $parametros);

        return $actualizado;
    } catch (PDOException $e) {
        // Manejar cualquier error de base de datos
        die("Error al actualizar detalles de la venta: " . $e->getMessage());
    } finally {
        // Cerrar la conexión
        $conexion = null;
    }
...
// Función auxiliar para obtener detalles de venta por ID
function obtenerVentaPorId($idVenta) {
    $sentencia = "SELECT * FROM ventas WHERE id = ?";
    $venta = select($sentencia, [$idVenta]);

    return ($venta) ? $venta[0] : null;
}
...
function eliminarVenta($idVenta) {
    try {
        ...
        $parametrosVenta = [$idVenta];
        $eliminadoVenta = ejecutarConsulta($sentenciaVenta, $parametrosVenta);

        // Eliminar los detalles de la venta (productos_ventas en lugar de detalles_ventas)
        $sentenciaDetalles = "DELETE FROM productos_ventas WHERE idVenta = ?";
        $parametrosDetalles = [$idVenta];
        $eliminadoDetalles = ejecutarConsulta($sentenciaDetalles, $parametrosDetalles);

        return $eliminadoVenta && $eliminadoDetalles;
    } catch (PDOException $e) {
        // Manejar cualquier error de base de datos
